...
class Vehicle {
  ...
  // Méthode pour vérifier si un clic se produit sur un véhicule ennemi
  clickedOnEnemy(enemies) {
    for (let i = enemies.length - 1; i >= 0; i--) {
      const enemy = enemies[i];
      const d = dist(mouseX, mouseY, enemy.pos.x, enemy.pos.y);
      if (d < enemy.r) {
        const laser = new Laser(this.pos.x, this.pos.y, enemy.pos);
        lasers.push(laser); // Ajouter le laser à un tableau global de lasers
        enemies.splice(i, 1); // Supprimer le véhicule ennemi touché
        break;
      }
    }
  }
  avoidLeader(target) {
    // Calculer le centre du cercle devant le leader
    let circleCenter = target.pos.copy();
    let circleRadius = 50; // Rayon du cercle, à adapter selon les besoins

    // Calculer la distance entre le véhicule actuel et le centre du cercle
    let distanceToCircle = this.pos.dist(circleCenter);

    // Si la distance est inférieure au rayon du cercle, le véhicule doit éviter cette zone
    if (distanceToCircle < circleRadius) {
      // Calculer la force d'évitement du cercle
      let avoidanceForce = p5.Vector.sub(this.pos, circleCenter);
      ...
      // Si le véhicule n'est pas dans la zone à éviter, renvoyer un vecteur nul
  ...
  avoid(obstacles) {
    ...
    if (Vehicle.debug) {
      ...
      noStroke();
      circle(
        pointAuBoutDeAhead2.x,
        pointAuBoutDeAhead2.y,
        this.rayonZoneDeFreinage
      );
    }
  ...
  avoidLeader(target) {
    ...
    if (distanceToCircle < circleRadius) {
      // Inversion de la direction de l'évitement
      let avoidanceForce = p5.Vector.sub(circleCenter, this.pos);
      avoidanceForce.setMag(this.maxSpeed);
      avoidanceForce.sub(this.vel);
      avoidanceForce.limit(this.maxForce);
...
class Target extends Vehicle {
  ...
  // Comportement "éviter la zone devant le leader"
  avoidLeaderZone(leaderPos, squareSize) {
    let aheadLeader = p5.Vector.add(
      leaderPos,
      leader.vel.copy().mult(squareSize)
    );
    let desired = p5.Vector.sub(aheadLeader, this.pos);
    let distance = desired.mag();

    if (distance < squareSize * 2) {
      desired.setMag(this.maxSpeed);
      desired.mult(-1); // Aller dans la direction opposée
  ...
  followLeader(target) {
    // Calculate a vector pointing from the position to the target's position
    let desired = p5.Vector.sub(target.pos, this.pos);

    // Distance at which to start evading the leader
    let evadeDistance = 50;

    // If the vehicle is within the evadeDistance, evade the leader
    if (desired.mag() < evadeDistance) {
      return this.evade(target);
    } else {
      // Otherwise, move towards the leader (seek behavior)
      return this.seek(target.pos);
    }
  }
  avoidLeaderZone(target, followers) {
    // Créer un cercle devant le leader
    let circleCenter = target.pos.copy();
    let circleRadius = 100; // Rayon de la zone à éviter devant le leader

    // Calculer la direction et la distance entre ce véhicule et le leader
    let toLeader = p5.Vector.sub(this.pos, target.pos);
    let distanceToLeader = toLeader.mag();

    // Calculer le vecteur de la zone à éviter devant le leader
    let avoidForce = createVector(0, 0);

    // Si ce véhicule est dans la zone à éviter
    if (distanceToLeader < circleRadius) {
      // Éviter la zone devant le leader
      avoidForce = p5.Vector.sub(this.pos, circleCenter);
  ...
  avoidLeaderZone(squareCenter, squareSize) {
    let avoidForce = createVector(0, 0);
    let distanceToSquare = p5.Vector.dist(this.pos, squareCenter);

    if (distanceToSquare < squareSize * 1.5) {
      // Éviter la zone carrée devant le leader
      let avoidVec = p5.Vector.sub(this.pos, squareCenter);
      avoidVec.setMag(this.maxSpeed);
      avoidVec.sub(this.vel);
      avoidVec.limit(this.maxForce);
      avoidForce.add(avoidVec);
    }
